...
void Oprea1Scaffold::GetScaffold(std::string &mol, RDKit::RWMol **scaff)
{
    *scaff = GetMol(mol);
    if (!*scaff) {
        return;
    ...
    while (removed) {
        ...
        for (aIter = (*scaff)->beginAtoms(); aIter != (*scaff)->endAtoms(); ++aIter) {
            atom = *aIter;
            if ((atom->getOwningMol().getRingInfo()->minAtomRingSize(atom->getIdx()) > 3) &&
                    (atom->getDegree() == 2)) {
                RemoveAtomAndJoinAllTwoNeighbors(atom, *scaff);
                removed = true;
                break;
...
ScaffoldSelector Oprea1Scaffold::GetSelector()
{
    return SF_OPREA_1;
}

std::vector<ChemOperSelector> Oprea1Scaffold::GetUsefulOperators()
{
