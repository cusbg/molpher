...
std::string GenerateSnaphostFilenameWithoutExtension(
    ...
    filename << base << "/" << std::setfill('0')
            << std::setw(3) << iterIdx;
    return filename.str();
}

std::string GenerateFilename(
    std::string &base, boost::uint32_t iter) {
    std::stringstream filename;
    filename << base << "/";
    if (iter < 10) {
        filename << "0";
...
//        assert(it != candidates.end());
//        smiles.push_back(it->second.smiles);
//        mols.push_back(&(it->second));
//    }
//
...
//                    << ";" << "Dist to etalon: " << mols.back()->distToEtalon << std::endl;
//            smiles.pop_back();
//            mols.pop_back();
//        }
//
...
void GatherMolphMols(const std::vector<MolpherMolecule> &toGather,
    ...
    for (it = toGather.begin(); it != toGather.end(); ++it) {
        gathered.insert(std::make_pair(it->smiles, *it)); // filter duplicates
    }
}
...
void WriteMolphMolsToSDF(const std::string &file,
    ...
    for (it = mols.begin(); it != mols.end(); ++it) {
        ...
        try {
            mol = RDKit::SmilesToMol(it->second.smiles);
            if (mol) {
...
void WriteMolphMolsToSDF(const std::string &file,
    ...
    for (it = mols.begin(); it != mols.end(); ++it) {
        RDKit::RWMol *mol = NULL;
        try {
            mol = RDKit::SmilesToMol(it->smiles);
            if (mol) {
                RDKit::MolOps::Kekulize(*mol);
            } else {
                throw ValueErrorException("");
...
//        mols.push_back(MolpherMolecule(smile));
//        delete mol;
//    }
//}
